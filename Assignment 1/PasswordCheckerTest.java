...
public class PasswordCheckerTest {
	...
	public void setUp() throws Exception {
		String[] p = {"334455BB", "Im2cool4U!", "george2ZZZ!", "4sale", "bertha22", "4wardMarch!", 
				...
				"myPassword2"}; //Added special char because that was thrown when shouldn't
	...
	{
		try{
			assertTrue(PasswordCheckerUtility.isValidPassword("abcABC12!")); //Changed this password because it throws
			PasswordCheckerUtility.isValidPassword("abc12");				 //NoSpecialCharacterException which causes fail test
	...
	{
		try{
			assertTrue(PasswordCheckerUtility.isValidPassword("1234567aA!")); //Changed this password because it throws
			PasswordCheckerUtility.isValidPassword("1234567a");				  //NoSpecialCharacterException which causes fail test
	...
	{
		try{
			assertEquals(true,PasswordCheckerUtility.isValidPassword("1234aaAA!")); //Added special char because wrong exception thrown when not supposed to
	...
	{
		try{
			assertEquals(true,PasswordCheckerUtility.isValidPassword("1234aaAA!")); //Changed this password because it throws
			PasswordCheckerUtility.isValidPassword("1234aAAA!");					//NoSpecialCharacterException which causes fail test
	...
	public void testValidPasswords() {
		...
		assertEquals(scan.next(), "george2ZZZ!"); //Added ! because specialchar exception is thrown
